...
$p \land q$& ``$p$ and $q$'' & is an abbreviation for & $\neg(p \Rightarrow (\neg q))$ \\
...
\begin{enumerate}[i]
  ...
  \item $\entails \left[p \Rightarrow (q \Rightarrow r)\right] \Rightarrow \left[(p \Rightarrow q) \Rightarrow (p \Rightarrow r) \right]$ 
    (not fun)
\end{enumerate}
